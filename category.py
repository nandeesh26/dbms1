...
class categoryclass:
    def __init__(self, root):
        ...
        label_name=Label(self.root,text="Enter Category Name",font=("times new roman",30),bg="white").place(x=50,y=100)
        ...
        self.image1=self.image1.resize((610,260),Image.ANTIALIAS)
    ...
    def add(self):
        ...
        try:
            if self.variable_name.get()=="":
                ...
                cur.execute("Select * from category where name=?",(self.variable_name.get(),))
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","This Category exists",parent=self.root)
                else:
                    cur.execute("Insert into category(name) values(?)",(self.variable_name.get(),
